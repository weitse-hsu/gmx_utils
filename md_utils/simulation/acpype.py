...
svnId = '$Id: acpype.py 403 2014-08-27 18:33:45Z alanwilter $'
try:
...
# verNum = re.sub('[^0-9\.]', '', sys.version.split()[0])  # SyntaxWarning
verNum = re.sub(r'[^0-9.]', '', sys.version.split()[0])
...
#        balanceChargeList = self.balanceCharges(chargeList)
    ...
    def getChirals(self):
        ...
        if self.obchiralExe:
            ...
            # print(cmd)
            # out = map(int, re.findall('Atom (\d+) Is', _getoutput(cmd)))  # SyntaxWarning
